...
import gsap from "gsap";
import { ScrollTrigger } from "gsap/ScrollTrigger";
import { usePageTransition } from "@/components/hook/usePageTransition";
...
const Home = () => {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      tl.to(leftDoorRef.current, { rotateY: -110, ease: "none" }, "scene")
        .to(rightDoorRef.current, { rotateY: 110, ease: "none" }, "scene")
        .to(doorContainerRef.current, { autoAlpha: 0, duration: 0.01 });

      tl.to(frontBuildingRef.current, { scale: 9, ease: "none" }, "zoom");
      tl.to(".BuildingElementL", { left: "0%", bottom: "-80%", ease: "none" }, "zoom");
      tl.to(".BuildingElementR", { right: "0%", bottom: "-80%", ease: "none" }, "zoom");

      tl.from(".wedding-subtitle", { y: 80, opacity: 0, scale: 0.8, ease: "back.out(1.7)" }, "zoom+=0.05");
      tl.from(".wedding-title", { y: 100, opacity: 0, scale: 0.7, ease: "back.out(1.7)" }, "zoom+=0.15");
      tl.from(".menu-item", { y: 80, opacity: 0, scale: 0.7, stagger: 0.15, ease: "back.out(1.7)" }, "zoom+=0.3");

      const weddingSubtitle = document.querySelector(".wedding-subtitle");
      if (weddingSubtitle) {
        weddingSubtitle.addEventListener("mouseenter", () =>
          gsap.to(weddingSubtitle, { y: -4, duration: 0.3, ease: "power2.out" })
        );
        weddingSubtitle.addEventListener("mouseleave", () =>
          gsap.to(weddingSubtitle, { y: 0, duration: 0.3, ease: "power2.out" })
        );
      }
      ...
      menuItemsEl.forEach((item) => {
        const icon = item.querySelector(".menu-icon");
        const enter = () => gsap.to(icon, { scale: 1.15, rotation: 8, duration: 0.4, ease: "power2.out" });
        const leave = () => gsap.to(icon, { scale: 1, rotation: 0, duration: 0.4, ease: "power2.out" });
        item.addEventListener("mouseenter", enter);
        item.addEventListener("mouseleave", leave);
        item._enter = enter;
        item._leave = leave;
      ...
      <div className="sticky top-0 left-0 h-screen w-full overflow-hidden">
        ...
        >
          <div className="relative flex h-full w-full">
            ...
            >
              <img src="/newImg/DOOR L_1_11zon.png" alt="" className="w-full object-cover object-top flex translate-x-[2px]" />
            </div>
            <div
              ref={rightDoorRef}
              className="h-full w-1/2"
            ...
            >
              <img src="/newImg/DOOR R_2_11zon.png" alt="" className="w-full object-cover object-top flex translate-x-[-2px]" />
            </div>
          </div>
        </div>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          ...
          {/* Menu — uses navigate() instead of Link */}
          <div className="grid w-fit grid-cols-4 gap-[3vw] max-sm:grid-cols-2">
            {menuItems.map((item, index) => (
              <div
                key={index}
              ...
              >
                ...
                <p className="menu-text F1 font-semibold text-[14px] uppercase max-w-[150px] text-center">
                  {item.title}
                </p>
              </div>
            ))}
          ...
          {/* Side Frames */}
          <div className="absolute left-0 flex h-screen max-sm:hidden w-1/4 justify-start">
            <img src="/newImg/L_IMG.png" alt="" className="h-full object-cover object-center" />
          </div>
          <div className="absolute right-0 flex h-svh max-sm:hidden w-1/4 justify-end">
